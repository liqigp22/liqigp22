...
//     保存数据
    ...
    val flow = sQLContext.sql("select count(sessionid)as ct ,provincename,cityname from Alldata group by provincename,cityname")

      flow.createOrReplaceTempView("dashabi")
...
//    flow.write.jdbc("jdbc:mysql://localhost:3306/dadaji","flow2",connectionProperties)
//    flow.write.partitionBy("provicename","cityname").json("hdfs://192.168.159.100/flow")
